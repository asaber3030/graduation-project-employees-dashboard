...
export const AdminCategoriesTable = ({ hasAccessToDelete, hasAccessToUpdate, showFilters = true, hasNextPage, searchParams, data }: Props) => {
  return (
    <div>
              ...
              {data.map((category) => (
                <TableRow key={`inventory-row-${category.id}`}>
                  ...
                  <TableCell className='text-right space-x-2'>
                    ...
                    {hasAccessToUpdate && <LinkBtn href={employeesRoutes.categories.update(category.id)} icon={Edit} />}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
